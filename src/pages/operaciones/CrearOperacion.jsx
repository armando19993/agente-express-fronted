...
const CrearOperacion = () => {
    ...
    const extractionConfig = {
        ...
        GIRO: [
            {
                pattern: /.*/,  // Coincide con cualquier texto
                field: 'banco_destino',
                value: "BCP"    // Valor fijo
            },
    ...
    const extractDynamicData = (text, operationType) => {
        ...
        config.forEach(({ pattern, field, groupIndex, value }) => {
            if (value !== undefined) {
                // Si hay un valor fijo (value), lo asignamos directamente
                updates[field] = value;
            } else {
                // Si no, procedemos con la extracción por patrón
